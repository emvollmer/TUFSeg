...
def merge_images(
    ...
    for channel in channels:
        ...
        if channel == "2ch":
            ...
            # save with TIR layer name as annotations were created with these
            np.save(str(out_path), out_2ch)
            _logger.debug(
                f"Merged 2 channel file saved to work directory ..."
                f"{str(Path(channel, end_of_path))}"
            )
        else:
            ...
            _logger.debug(
                f"Merged 4 channel file saved to work directory ..."
                f"{str(Path(channel, end_of_path))}"
            )
