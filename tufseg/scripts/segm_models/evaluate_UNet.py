...
def parse_args():
    ...
    parser.add_argument('model_root', metavar='MODEL_ROOT',
                        help='Path to destination directory that contains model and model configs.')
...
#        default_log: bool = False,
    ...
    sm_mean_scores = evaluate_sm(X_test, y_test, model=model, NUM_CLASSES=NUM_CLASSES)
    combined_mean_scores, combined_class_mean_scores = (
        evaluate_sklearn_imagewise(X_test, y_test, model=model)
    )
    holistic_mean_scores = evaluate_sklearn_holistic(X_test, y_test, model=model)
    ...
    with open(eval_json_path, "w") as json_file:
        json.dump(
            {"sm metrics": sm_mean_scores,
             ...
             "sklearn metrics - combined imagewise results": combined_mean_scores,
             "sklearn metrics - combined imagewise classwise results": combined_class_mean_scores,
             },
            json_file, indent=4)
...
def evaluate_sm(X_test, y_test, model, NUM_CLASSES):
    ...
    for key, val in mean_scores.items():
        print(f"{key.ljust(max(len(k) for k in mean_scores.keys()))}: {round(val, 4)}")
...
def evaluate_sklearn_imagewise(X_test, y_test, model):
    ...
    """
    _logger.info("Evaluate model by evaluating test images individually and combining scores.")
    ...
    for test_img, test_mask in tqdm(zip(X_test, y_test)):
        ...
        for metric_name, metric_attrib in sklearn_metrics.items():
            ...
            score = function(test_mask.flatten(), class_predictions.flatten(), **metric_params)
            total_scores[metric_name].append(score)

            # skip weighted and balanced eval functions for class-wise evaluation
            if 'weighted' in metric_name.lower() or 'balanced' in metric_name.lower():
                continue
            ...
            for class_id, class_name in enumerate(classes):
                ...
                if np.any(class_mask_true) or np.any(class_mask_pred):
                    ...
                    class_score = function(class_mask_true.flatten(), class_mask_pred.flatten(),
                                           **metric_singleclass_params)
                    classwise_scores[metric_name][class_name].append(class_score)
    ...
    for metric_name in sklearn_metrics.keys():
        if 'weighted' not in metric_name.lower() and 'balanced' not in metric_name.lower():
            classwise_metric_name = f'mean_{metric_name}_classwise'
            classwise_scores_dict = {class_name: np.mean(scores)
                                     for class_name, scores in classwise_scores[metric_name].items()}
            mean_classwise_scores[classwise_metric_name] = classwise_scores_dict

    print("------------------- class-wise evaluation results:")
    for key1, val1 in mean_classwise_scores.items():
        print(f"{key1}")
        for key2, val2 in val1.items():
            print(f"{key2.ljust(max(len(k) for k in val1.keys()))}: "
                  f"{round(val2, 4)}")
...
def evaluate_sklearn_holistic(X_test, y_test, model):
    ...
    :return: mean_scores: (dict) evaluation metrics applied to model
    """
    _logger.info("Evaluate model by evaluating all images together and calculating scores.")
    ...
    for name, attrib in config['eval']['SKLEARN_METRICS'].items():
        ...
        mean_score = function(y_test.flatten(), all_class_predictions.flatten(), **metric_params)
