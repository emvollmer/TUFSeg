...
from tufseg.scripts.configuration import init_temp_conf, update_conf, cp_conf
...
NUM_CLASSES = len(config['data']['masks']['labels']) + 1    # +1 for background
...
def parse_args():
    ...
    parser.add_argument('-dst', '--dst-model-dir', dest='model_root',
                        required=True,
    ...
    parser.add_argument('-src', '--src-data-dir', dest='data_root',
                        help='(Optional) Path to source directory of all data.'
                             ' If not provided, value from temp config '
                             '(defined during setup) will be used.')
    parser.add_argument('-ch', '--channels', dest='channels',
                        type=int, default=4, choices=[4, 3],
                        help='Whether to process the data as 3 channels '
                             '(greyRGB+T+T) or keep the 4 channels (RGBT).')
    parser.add_argument('-proc', '--processing', dest='processing',
                        type=str, default="basic",
                        choices=["basic", "vignetting", "retinex_unsharp"],
                        help='Whether to apply additional filters to the data '
                             '(retinex unsharp, vignetting removal) or '
                             'keep as basic (RGBT).')
    parser.add_argument('--only-tir', dest='only_tir', action='store_true',
                        help='Whether to use only thermal image (not RGB). '
                             'If the flag is added, the inputs will '
                             'automatically be 3 channels and the selected '
                             'processing option is applied.')
    parser.add_argument('-cfg', '--cfg-options', dest='cfg_options', nargs='+',
                        action=GetKeyValuePairs,
                        help='Specify training cfg options in the following '
                             'manner: "seed=1000 lr=0.001 epochs=35 '
                             'batch_size=8". Without these, the listed '
                             'examples will be chosen as defaults')
...
def main(
    ...
    main_args = locals()
    if (
            main_args.get('cfg_options', {}).get('encoded_weights')
            in ["None", "none"]
    ):
    ...
    model_path = Path(
        model_dir, config['model']['type'] + config['model']['suffix']
    )
    model = train(
...
class CustomEpochLogger(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        log_message += (
            'Validation: '
            + " - ".join([f"{k}: {round(v, 4)}" for k, v in val_logs.items()])
            + '\n'
        )
...
def train(X_train, y_train_onehot, X_test, y_test_onehot, model_path: Path):
    ...
    X data can be 3 channel or of different dimension (UNet will automatically
    be adapted).
    ...
    if not isinstance(model_path, PosixPath) or model_path.suffix != ".hdf5":
        raise FileNotFoundError(
            f"Provided model path '{model_path}' is not a PosixPath ending "
            f"in '.hdf5'!"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Loss function '{loss_name}' not found "
            f"in tensorflow_addons.losses!"
        )
    ...
    try:
        ...
        raise ValueError(
            ...
            f"in tensorflow.keras.optimizers!"
        )

    _logger.info(
        f"Compiling model with '{loss_name}' loss function "
        f"and '{optimizer_name}' optimizer."
    )
    model.compile(
        optimizer=optimizer(learning_rate=cfg['optimizer']['lr']),
        loss=loss_function(alpha=cfg['loss']['alpha'],
    ...
    CSVHistoryLogger = keras.callbacks.CSVLogger(
        Path(model_path.parent, 'training_history.csv'),
